...
# Gitignore template-provided workflow files so downstream projects
# don't re-commit template internals. The files stay on disk (Claude
# needs them) but are untracked in the downstream project's git.
GITIGNORE_MARKER="# --- agent-workflow-template: template-provided files ---"
if ! grep -qF "$GITIGNORE_MARKER" .gitignore 2>/dev/null; then
    cat >> .gitignore << 'IGNORE'

# --- agent-workflow-template: template-provided files ---
# These are shipped by the template and needed at runtime, but should
# not be tracked in a downstream project's git history. Remove this
# block (and re-run `git add`) if you want to vendor them instead.
/scripts/agentic/
/.skills/
/docs/personas/
/docs/sprints/_templates/
/docs/initiatives/_templates/
/.claude/settings.permissive.json
/setup.sh
/GEMINI.md
IGNORE
    echo "  ✓ Appended template-file ignores to .gitignore"

    # If this directory is already a git repo with these files tracked
    # (e.g. cloned directly from the template), untrack them so the
    # new ignore rules take effect. Files remain on disk.
    if [ -d .git ]; then
        git rm -r --cached --quiet \
            ...
            .claude/settings.permissive.json \
            setup.sh \
            GEMINI.md 2>/dev/null || true
        echo "  ✓ Untracked template files from git index (still on disk)"
    fi
fi
