...
**8. Lane integrity — two automatic BLOCKERs (added 2026-08-10, from a downstream RCA).** (Checklist item 8 of this §6b list — distinct from §8 "Architecture Lane Adherence" below, which is the sprint-accept lane review.) These are not judgement calls; if the diff shows either, the finding is a BLOCKER regardless of how good the change is:
