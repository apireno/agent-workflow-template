...
- **When training data, gold corpora, or evaluation datasets may contain PII, customer data, or proprietary content:** consult VP of Data Science (`— Data`) on dataset lineage, sampling discipline, and what's actually flowing into the training/eval pipeline. The security and statistical concerns intersect — VP DS knows which subsets of the data are actually exercised by which experiments. Reference: `docs/personas/vp-datascience.md`.
...
Put project specifics in **`docs/personas/context/vp-security-context.md`**, which every sync explicitly
