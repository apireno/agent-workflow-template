...
- **For ML/statistical systems:** monitoring coverage MUST include evaluation-metric stability over time (drift detection) and dataset-version provenance for any deployed model. Coordinate with VP of Data Science (`— Data`) on which metrics need replay/reproducibility instrumentation and what the alert thresholds should be — a metric regressing 2% might be noise or might be the canary for a silent training data corruption, and VP DS owns that distinction. Reference: `docs/personas/vp-datascience.md`.
...
Put project specifics in **`docs/personas/context/vp-devops-context.md`**, which every sync explicitly
