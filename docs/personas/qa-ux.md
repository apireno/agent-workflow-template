...
- **One named lane per drive — mint+name+navigate in ONE call, reuse the id, close at the end.** DOMShell drives the operator's **real** Chrome; isolation is by **group** (a lane), *not* a separate browser — and `group close <id>` is **group-scoped** (it closes only that group's tabs, never the user's). Per DOMShell ≥2.0.5, your **first** `domshell_execute` passes `group_id:"new"` + `initial_url:"<url>"` + `group_name:"qa-ux-<repo>-<sprint>"` **together** (opens the URL at creation — no separate nav to fail — and tags the group). Capture the returned `[lane: <id>]`, **record it**, and **reuse that id on every later call** — call `group_id:"new"` **exactly once per drive**. If a navigation is rejected, retry **with the same id** (use `open <url>`, not `cd <url>`) — **never re-mint a group** (re-minting strands an orphan; that is the leak bug). **Close it as your final action: `group close <id>`.** **NEVER operate in `group_id:"shared"` or with `group_id` omitted** — per DOMShell #53 (ext ≥1.3.2) those mean *"the user's REAL browser, no isolated lane"*, so any **write** (`open`/`click`/`type`/`select`/`submit`/`js`/`key`) there hits the operator's actual tabs, not your sandbox. Your named `group_id:"new"` lane is the only safe place to drive; `shared` is acceptable ONLY for a read-only `group list`.
...
## The living master baseline + per-sprint regression subset

The per-sprint `qa-plan.md` covers *this sprint's* journeys. Above it sits the **living master baseline** — `docs/qa/ux-baseline-test-plan.md` — the **canonical UX acceptance bar** for the whole product: every user-facing feature, each traced to its PRD/requirement, with a **coverage matrix** (`driven` / `not-driven` / `not-built`) and a **code-touchpoints** column (module / route / `data-action`) so a sprint touching module X can mechanically resolve the test IDs it impacts. **QA-UX owns authoring and amending the master baseline; VP-Product owns approving it** (it is the product acceptance bar — see `vp-product.md`).

- **Promote → living doc.** Once a comprehensive baseline is authored and VP-Product-approved, it is promoted from the sprint dir to the stable `docs/qa/` path with a governance header. It grows **monotonically** and never silently drifts.
- **Per-sprint regression subset (not the whole baseline).** At planning, the sprint's impacted feature set (from the code it will touch) maps — via the code-touchpoints column — to a **subset of baseline test IDs**. That subset is the sprint's accept-time regression gate: re-driven (or its pinned `tests/e2e/` scripts run) at `/sprint-accept`. The **full** baseline is re-driven only on cadence (release / major refactor), never every sprint.
- **Amendment is a sprint deliverable.** A sprint that adds or changes a feature **amends the master baseline** (new/changed tests + coverage-matrix status) as part of its deliverables, VP-Product-approved alongside the dev-report.

So there are three QA artifacts at three scopes: the **master baseline** (repo-level, durable acceptance bar) → the per-sprint **`qa-plan.md`** (this sprint's regression subset + any new tests) → the **`qa-report.md` + flow-graph** (what this drive actually found).
...
- **Bright-line (non-negotiable):** the QA-UX *engine* is **interactive Claude** (subscription pool) — **never `claude -p` / the Anthropic API.** Default (and currently only) driver-engine = **interactive Claude** via `qa-drive-claude.sh`. **The former gemini engine is DEPRECATED/DEAD as of 2026-06-19** — Google removed the gemini-CLI free tier (`IneligibleTierError` / `UNSUPPORTED_CLIENT`, "migrate to Antigravity"), so gemini can no longer authenticate. (This same gemini death also disables `/vp-review` + `/sprint-fanout` fleet-wide — a separate CTO/CEO engine decision.) DOMShell's single-tool interface means one MCP approval, no per-tool friction.
...
- **Cleanup is PROVENANCE-based — close what you minted, never GC by heuristic.** There is no reliable "garbage" signal for a tab group: `about:blank` disappears once groups open `initial_url` at creation, and "long-lived" is *not* garbage (it may be a session a human walked away from and wants to resume). So **never auto-GC by age/state.** Instead: the drive **names** its group (`qa-ux-<repo>-<sprint>`) and **records every lane id it mints** to `<sprint-dir>/.qa-lanes`. Three layers, all closing only the run's *own* recorded ids (group-scoped, idempotent, safe):
  1. the agent closes its lane as its final action;
  2. the drive harness traps **EXIT/INT/TERM** and closes **every recorded id** even on crash/timeout/kill, then reaps the proxy registration;
  3. the **invoker** (the agent that fired the round) runs `scripts/agentic/qa-cleanup-groups.sh` **after the round** — it closes the round's recorded `.qa-lanes` ids and, on DOMShell extension ≥1.3.2 (named group titles), also sweeps any group named `qa-ux-*` (catching orphans whose drive died before recording). It never touches a group it didn't create, so a human's walked-away session is safe. This is robust to a 403'd/dead engine because the invoker runs it independently.
...
- **Sanctioned tooling only — a tool gap is an escalation, never a workaround.** A drive uses the **one provisioned automation lane** for each surface (the project's declared browser-automation MCP, the declared CLI entrypoint, the repo's own MCP). Three rules follow, and they are binding:
  1. **Never self-provision an alternate driver** — not another browser-automation extension, not a hand-rolled scripted browser, not a second MCP that happens to be available in the session. A second lane means findings are no longer comparable across runs, and it silently changes what the gate is actually testing. The harness enforces the primary case by denying the alternate browser tools outright; the rule binds everywhere the harness cannot.
  2. **A perceived tool gap is escalated with evidence** — name the surface, the action attempted, the exact failure — and the drive stops there. "The sanctioned tool couldn't do it so I used another one" is a lane violation, and it hides a real tooling defect the fleet needs to fix.
  3. **A rendering or behavioural anomaly is a PRODUCT BUG until proven otherwise.** File it and RCA it. Assuming "that's just the automation tool being weird" is how real defects get discarded — the tool is the *instrument*, and blaming the instrument for the reading requires evidence, not a hunch.
